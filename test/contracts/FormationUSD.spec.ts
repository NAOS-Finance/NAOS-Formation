...
describe("FormationUSD", () => {
  ...
  before(async () => {
    ...
    YearnVaultMockUsdFactory = await ethers.getContractFactory(
      "YearnVaultMockUSD"
    );
  ...
  describe("vault actions", () => {
    ...
    describe("migrate", () => {
      beforeEach(async () => {
        ...
        vaultMock = (await YearnVaultMockUsdFactory.connect(deployer).deploy(
          token.address,
          controllerMock.address
        )) as YearnVaultMockUsd;
      ...
      context("when caller is current governance", () => {
        ...
        context("when adapter token mismatches", () => {
          ...
          beforeEach(async () => {
            invalidAdapter = (await YearnVaultMockUsdFactory.connect(
              deployer
            ).deploy(
              tokenAddress,
              controllerMock.address
        ...
        context("when conditions are met", () => {
          beforeEach(async () => {
            newAdapter = (await YearnVaultAdapterFactory.connect(
              deployer
            ).deploy(
              vaultMock.address,
              formation.address
    ...
    describe("recall funds", () => {
      ...
      context("from an inactive vault", () => {
        ...
        beforeEach(async () => {
          ...
          inactiveAdapter = (await YearnVaultAdapterFactory.connect(
            deployer
          ).deploy(vaultMock.address, formation.address)) as YearnVaultAdapter;

          activeAdapter = (await YearnVaultAdapterFactory.connect(
            deployer
          ).deploy(vaultMock.address, formation.address)) as YearnVaultAdapter;
      ...
      context("when there is at least one vault to flush to", () => {
        context("when there is one vault", () => {
          ...
          beforeEach(async () => {
            ...
            vaultMock = (await YearnVaultMockUsdFactory.connect(
              deployer
            ).deploy(
              token.address,
              controllerMock.address
        ...
        context("when there are multiple vaults", () => {
          ...
          beforeEach(async () => {
            ...
            activevaultMock = (await YearnVaultMockUsdFactory.connect(
              deployer
            ).deploy(
              token.address,
              controllerMock.address
            ...
            inactivevaultMock = (await YearnVaultMockUsdFactory.connect(
              deployer
            ).deploy(
              token.address,
              controllerMock.address
    ...
    describe("deposit and withdraw tokens", () => {
      ...
      it("revert with withdraw too much tokens after repay with nUSD", async () => {
        ...
        await formation.connect(minter).repay(0, mintAmt);
        expect(
          formation.connect(minter).withdraw(depositAmt.add(parseUnits("1", 1)))
        ).revertedWith("Exceeds withdrawable amount");
      });
      ...
      it("revert with repay too much USDT", async () => {
        ...
        expect(
          formation
            .connect(minter)
            .repay(repayAmtUSD.add(parseUnits("1", 1)), 0)
        ).revertedWith("");
      });
    ...
    describe("repay and liquidate tokens", () => {
      ...
      beforeEach(async () => {
        ...
        await nUsd
          .connect(minter)
          .approve(transmuterContract.address, depositAmt.mul(USDT_CONST));
      });
      it("repay with USDT reverts when nothing is minted and transmuter has no nUsd deposits", async () => {
        await formation
          .connect(minter)
          .deposit(depositAmt.sub(parseUnits("1000", 6)));
        expect(formation.connect(minter).repay(repayAmtUSD, 0)).revertedWith(
      ...
      it("liquidates the minimum necessary from the formation buffer", async () => {
        ...
        expect(formationTokenBalPost).equal(
          dep2Amt.sub(liqAmt.div(USDT_CONST))
        );
        expect(transmuterEndingTokenBal).equal(liqAmt.div(USDT_CONST));
      });
      ...
      it("deposits, mints nUsd, repays with nUsd and USDT, and has no outstanding debt", async () => {
        ...
        await formation
          .connect(minter)
          .repay(parseUnits("500", 6), parseEther("500"));
        expect(
          await formation
            .connect(minter)
            .getCdpTotalDebt(await minter.getAddress())
      ...
      it("deposits and liquidates USDT", async () => {
        ...
        expect(
          await formation
            .connect(minter)
            .getCdpTotalDeposited(await minter.getAddress())
        ).equal(depositAmt.sub(mintAmt.div(USDT_CONST)));
      });
    });
    ...
    describe("mint", () => {
      ...
      context("is whiltelisted", () => {
        ...
        it("reverts when trying to mint too much", async () => {
          ...
          await nUsd
            .connect(deployer)
            .setCeiling(formation.address, parseEther("5000"));
          expect(
            formation
              .connect(minter)
              .mint(parseEther("2500").add(parseUnits("1", 1)))
    ...
    describe("harvest", () => {
      ...
      it("harvests yield from the vault", async () => {
        ...
        expect(
          transmuterBal.sub(yieldAmt.sub(yieldAmt.div(pctReso / harvestFee)))
        ).to.be.at.most(1);
      ...
      it("sends the harvest fee to the rewards address", async () => {
        ...
        expect(
          yieldAmt.div(pctReso / harvestFee).sub(rewardsBal)
        ).to.be.at.most(10000000);
      });
