...
contract EllipsisVaultAdapter is IVaultAdapterV2 {
  ...
  /// @dev threeesToken
  IDetailedERC20 public threeesToken;
  ...
  constructor(I3ESPool _vault, address _admin, IUniswapV2Router02 _uniV2Router, IEllipsisPool _stakingPool, IDetailedERC20 _threeesToken, IDetailedERC20 _ellipsisToken, IDetailedERC20 _wBNBToken, uint256 _stakingPoolId, uint256 _assetId) public {
    ...
    require(address(_threeesToken) != address(0), "EllipsisVaultAdapter: _ellipsisToken cannot be 0x0.");
    ...
    threeesToken = _threeesToken;
    wBNBToken = _wBNBToken;
    stakingPoolId = _stakingPoolId;
    assetId = _assetId;

    updateApproval();
    decimals = threeesToken.decimals();
  ...
  function totalValue() external view override returns (uint256) {
    uint256 amount = threeesToken.balanceOf(address(stakingPool));
  ...
  function withdraw(address _recipient, uint256 _amount) external override onlyAdmin {
    ...
    // withdraw
    vault.remove_liquidity_one_coin(_tokensToShares(_amount), int128(assetId), _tokensToShares(_amount));
